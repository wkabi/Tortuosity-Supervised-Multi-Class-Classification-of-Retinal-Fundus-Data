# this is taken for comparison from here: https://github.com/HzFu/EyeQ/tree/master/EyeQ_preprocess
import cv2
import numpy as np
...
def get_only_mask(img):
    mask, _, _, _ = get_mask(img)
    return mask

def get_only_mask_2(img):
    # mask, _, _, _ = get_mask(img)
    # preprocess images
    #   img : origin image
    #   tar_height: height of tar image
...
def crop_to_fov(img):
    # create fov mask
    img = np.array(img)
    fov = get_only_mask(img)

    fov = np.array(fov).astype(int)
    minr, minc, maxr, maxc = regionprops(fov)[0].bbox
    im_crop = np.array(img)[minr:maxr, minc:maxc]
