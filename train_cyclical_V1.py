...
parser.add_argument('--csv_train', type=str, default='data/DR_train_20210203.csv', help='path to training data csv')
parser.add_argument('--data_path', type=str, default='data/cropped_images/', help='path to training data csv')
parser.add_argument('--model_name', type=str, default='bit_resnext50_1', help='architecture')
parser.add_argument('--n_classes', type=int, default=6, help='batch Size')
...
parser.add_argument('--pretrained_weights', type=str, default=False, help='start from eyepacs-pretrained weights (path to)')
...
def run_one_epoch(loader, model, criterion, do_mixup=0., scheduler=None, optimizer=None, assess=False):
...
def train_model(model, optimizer, train_criterion, val_criterion, do_mixup, train_loader, val_loader,
    ...
    if model.n_classes == 5: class_names = ['0', '1', '2', '3', '4']
    elif model.n_classes == 3: class_names = ['DME0', 'DME1', 'DME2']
    elif model.n_classes == 4: class_names = ['0', '1', '2', '3']
    else:  class_names = ['C{}'.format(i) for i in range(len(model.n_classes))]
    ...
    for cycle in range(n_cycles):
        ...
        if model.n_classes == 3:
            print('AUC: DR0={:.2f}|{:.2f} - DR1={:.2f}|{:.2f} - DR2={:.2f}|{:.2f}'.format(
                    100 * tr_auc_all[0], 100 * vl_auc_all[0], 100 * tr_auc_all[1], 100 * vl_auc_all[1],
...
if __name__ == '__main__':
    ...
    if optimizer_choice == 'adam':
        ...
        optimizer = sgd_optimizer(model, lr=max_lr, momentum=0, weight_decay=args.wd)
    ...
    elif optimizer_choice == 'madgrad':
        optimizer = madgrad(model.parameters(), lr=max_lr, weight_decay=args.wd)
    else:
        sys.exit('please choose a valid optimizer')
    if pretrained_weights is True:
        weights_path = osp.join('experiments/cyclical/bit50_OS_12121_51_BS8_sam_600x600/cycle_10_mAUC_94.18_MCC_65.44_F1_55.68/')
        try:
    ...
    tr_mccs, vl_mccs, tr_f1s, vl_f1s, tr_ls, vl_ls, b_cycle=train_model(model, optimizer, train_criterion, val_criterion, do_mixup,
    ...
    if do_not_save is False:
        with open(osp.join(experiment_path, 'val_metrics.txt'), 'w') as f:
            ...
            for j in range(len(vl_aucs)):
                ...
                if model.n_classes==5:
                    print('AUC: 0={:.2f}/{:.2f}, 1={:.2f}/{:.2f}, 2={:.2f}/{:.2f}, '
                          '3={:.2f}/{:.2f}, 4={:.2f}/{:.2f}'.format(
                    ...
                    print('AUC: 0={:.2f}/{:.2f}, 1={:.2f}/{:.2f}, 2={:.2f}/{:.2f}, '
                          '3={:.2f}/{:.2f}'.format(
                           100*tr_aucs[j][0], 100*vl_aucs[j][0], 100*tr_aucs[j][1], 100*vl_aucs[j][1], 100*tr_aucs[j][2], 100*vl_aucs[j][2],
                           100*tr_aucs[j][3], 100*vl_aucs[j][3]), file=f)
                elif model.n_classes == 3:
